...
void CudaSimulation::setSimulationParameters(SimulationParameters const& parameters)
{
...

    {
        ...
        parameters.cellFunctionWeaponEnergyCost = 0;
        ...
        parameters.radiationFactor = 0.0001f;
        parameters.radiationProb = 0.03f;
        parameters.radiationVelocityMultiplier = 1.0f;
        parameters.radiationVelocityPerturbation = 0.5f;

        CHECK_FOR_CUDA_ERROR(cudaMemcpyToSymbol(
            cudaSimulationParametersSpot, &parameters, sizeof(SimulationParameters), 0, cudaMemcpyHostToDevice));
    }
}
